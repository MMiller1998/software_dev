...
class RefereeTests(unittest.TestCase):
    ...
    def test_referee_constructor_valid(self):
        ...
        Referee(self.MOCK_TRAIN_MAP, players, Referee.STARTING_NUM_RAILS, mock_card_strategy, mock_dest_strategy)
        mock_card_strategy.create_deck.assert_called_with(Referee.STARTING_DECK_SIZE)

    def test_referee_constructor_invalid(self):
        mock_card_strategy, mock_dest_strategy = self.setup_strategy_mocks(MagicMock(), MagicMock())
        with self.assertRaises(ValueError):
            Referee(self.MOCK_TRAIN_MAP, [], 45, mock_card_strategy, mock_dest_strategy)

    def test_referee_run_game_stale_end(self):
        card_strategy_mock = MagicMock()
        card_strategy_mock.create_deck.return_value = []
        player_1 = Player(HoldStrategy)
        player_2 = Player(HoldStrategy)
        player_3 = Player(HoldStrategy)
    ...
    def test_referee_run_game_cheaters_end(self):
        card_strategy_mock = MagicMock()
        card_strategy_mock.create_deck.return_value = []
        player_1 = Player(ExplodeStrategy)
        player_2 = Player(ExplodeStrategy)
        player_3 = Player(ExplodeStrategy)
        player_list = [player_1, player_2, player_3]
        game_result = Referee.run_game(self.TRAIN_MAP, player_list, deck_creation_strategy=card_strategy_mock)
    ...
    def test_referee_run_game_termination_condition(self):
        ...
        player_1 = Player(BuyNowStrategy)
        player_2 = Player(BuyNowStrategy)
        player_3 = Player(BuyNowStrategy)
        player_list = [player_1, player_2, player_3]
    ...
    def test_setup_game(self, mock_dest_strategy, mock_card_strategy, mock_private_player_state, mock_ref_game_state):
        ...
        ref = Referee(self.MOCK_TRAIN_MAP, players, deck_creation_strategy=mock_card_strategy_instance,
                      destination_options_strategy=mock_dest_strategy_instance)
        self.assertEqual(250, len(ref._Referee__cards))
        ref.setup_game()
        self.assertEqual(238, len(ref._Referee__cards))
        for player, destinations in zip(players, [player_1_destinations, player_2_destinations, player_3_destinations]):
            player.setup.assert_called_with(self.MOCK_TRAIN_MAP, Referee.STARTING_NUM_RAILS, [Color.RED] * 4)
    ...
    def test_end_game(self, mock_game_result, mock_ref_game_state):
        ...
        ref = Referee(self.MOCK_TRAIN_MAP, players, deck_creation_strategy=MagicMock(), destination_options_strategy=MagicMock())
