...
class RandomDestinationOptionsStrategyTests(unittest.TestCase):
    ...
    def test_order_destinations(self):
        destinations = LexiSortedDestinationOptionsStrategy().order_destinations(
            {self.DEST_3, self.DEST_2, self.DEST_5, self.DEST_4, self.DEST_1})

        self.assertEqual(5, len(destinations))
        self.assertEqual(self.DEST_1, destinations[0])
        self.assertEqual(self.DEST_2, destinations[1])
        self.assertEqual(self.DEST_3, destinations[2])
        self.assertEqual(self.DEST_4, destinations[3])
        self.assertEqual(self.DEST_5, destinations[4])
